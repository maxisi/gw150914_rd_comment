...
% Here we show that those conclusions were based on a flawed analysis of the LIGO data, and on erroneous expectations for the evolution of the overtone amplitude posterior as a function of analysis start time.
Here we point out a number of technical errors in that analysis, including a software bug, and show that features
% of the overtone posterior
...
%Altogether, these findings undermine the central conclusions in \cite{Cotesta:2022pci}.
...
% This can only mean that either the posterior was not correctly sampled, or that the two analyses were not based on the same data.

%Starting from the configuration file released in \cite{Cotesta:2022pci},
...
%(Hanford GPS times of 1126259462.423 s or 1126259462.42323 s, respectively):
...
%, the timescale over which the $A_1$ posterior evolves is determined not only by the decay of the tone, but by the noise.
...
% The related objection ``that noise can indeed induce artificial evidence for an overtone'' is simply stating the basic fact that noise necessarily shifts the inferred $A_1$ value around. The important point is that the probability of a given fluctuation is quantified by the posterior, when computed correctly.
% % For any given set of data, the posterior accounts for this probability by construction.
% % In expectation, Gaussian noise will mimic a $1\sigma$ detection 32\% of the time, a $2\sigma$ detection 5\% of the time, and so on; this is consistent with the injections in \cite{Cotesta:2022pci,Isi:2022mhy,Finch:2022ynt}.
